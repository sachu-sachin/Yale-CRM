...
import { dealStatusLabels, adPhaseLabels, leadSourceLabels } from "@/lib/utils";

interface Telecaller { id: string; name: string; }

const REPORT_TYPES = [
  { key: "deals", label: "Deals" },
  { key: "payments", label: "Payments (Paid)" },
...
export default function AdminReportsPage() {
  ...
  const showDealFilters = type === "deals" || type === "payments";
  ...
  const download = () => {
    ...
    if (type === "deals" && status) p.set("status", status);
    if (showDealFilters && phase) p.set("phase", phase);
    if (showDealFilters && source) p.set("source", source);
    ...
    <>
      ...
      <div className="p-6 max-w-3xl">
        <div className="bg-white rounded-2xl border border-slate-100 p-6">
            ...
            {REPORT_TYPES.map((r) => (
              <button key={r.key} onClick={() => setType(r.key)}
                className={"px-4 py-2 rounded-xl text-sm font-medium transition-colors " +
                  (type === r.key ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/20" : "bg-slate-50 border border-slate-200 text-slate-600 hover:bg-slate-100")}>
                {r.label}
              </button>
            ))}
          </div>
            ...
            {showDealFilters && (
              <>
                ...
                <div>
                  ...
                  <select value={source} onChange={(e) => setSource(e.target.value)} className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/30">
          ...
          <button onClick={download} className="mt-6 flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-2.5 bg-emerald-600 text-white rounded-xl text-sm font-semibold hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-500/20">
